...
for each_topic in TOPIC_DICT:
    ...
    for eachele in TOPIC_DICT[each_topic]:
        try:
            ...
            CURRENTTITLE = eachele[0].replace("-","_").replace(" ","_").replace("/","")
            CURRENTINDEX = str(index_counter)
            NEXTINDEX = str(index_counter + 1)
            index_counter += 1
            
            print (FUNC_TEMPLATE.replace("CURRENTTOPIC",CURRENTTOPIC).replace("CURRENTINDEX",CURRENTINDEX).replace("NEXTINDEX",NEXTINDEX).replace("CURRENTTITLE",CURRENTTITLE).replace("CURRENTHTML",CURRENTHTML))
