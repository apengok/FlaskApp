...
for each_topic in TOPIC_DICT:
    ...
    for eachele in TOPIC_DICT[each_topic]:
        try:
            ...
            CURRENTTITLE = eachele[0].replace("-","_").replace(" ","_").replace("/","").replace(":","").replace(".","").replace(",","").replace("!","").replace("(","").replace(")","").replace("'","").replace("?","")
            CURRENTINDEX = str(index_counter)
            NEXTINDEX = str(index_counter + 1)
            index_counter += 1
            
            print TOPIC_DICT[each_topic][int(CURRENTINDEX)][1]
